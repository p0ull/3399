...
int csc_init(void) {
   ...
   // printArray(host_arr, NUM_ELEMENTS);
    ...
    gpu_rgb = clCreateBuffer(context, CL_MEM_HOST_READ_ONLY|CL_MEM_WRITE_ONLY | CL_MEM_ALLOC_HOST_PTR,
                                        NUM_ELEMENTS * sizeof(char), NULL, &ret);
   //gpu_yuv = clCreateBuffer(context, CL_MEM_HOST_WRITE_ONLY | CL_MEM_READ_ONLY |CL_MEM_ALLOC_HOST_PTR,
                                       // NUM_ELEMENTS * sizeof(char), NULL, &ret);
...
//    gettimeofday(&start,NULL);
	//   printf("dmafd=%d\n",dmafd);
	    const cl_import_properties_arm props[3] = {
	      ...
	      gpu_yuv = clImportMemoryARM(context,CL_MEM_READ_WRITE,props,&dmafd,1920 * 1080 * 2,&error);
	      if(error!=CL_SUCCESS){
	    	  printf("clImportMemoryARM=%d\n",error);
	      }else{
	    	 // printf("clImportMemoryARM= CL_SUCCESS\n");
	    	  clSetKernelArg(kernel, 1, sizeof(cl_mem), &gpu_yuv);
	      }
...
//    //printf("\ngpu_cpu_yuv=%p\n",gpu_cpu_yuv);
//
//    memcpy(cpu_rgb,cpu_yuv, width*height*3/2);
//    //ret = clFlush(command_queue);
//    //ret = clFinish(command_queue);
//
