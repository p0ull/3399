...
int convert_release_picture(char *p);
void convert_boot_init(float ratio);

char *gstreamer_get_picture(void *p)
{
	return convert_get_picture(p);
}

int gstreamer_release_picture(char *p)
{
	return convert_release_picture(p);
}
...
video_init (const char *filename)
{
  ...
  GstPad *pad;
  GstBus *bus;
  static char pipeline[1024];

  dec = g_new0 (struct decoder, 1);
  dec->loop = g_main_loop_new (NULL, FALSE);

  /* Setup pipeline: */
  //sprintf(pipeline,"uridecodebin uri=%s  ! videoconvert ! video/x-raw,format=BGR! appsink sync=false name=\"sink\"",filename);
  //sprintf(pipeline,"uridecodebin uri=%s  ! videoscale ! video/x-raw,width=640,height=480 ! appsink sync=false name=\"sink\"",filename);
 // sprintf(pipeline,"uridecodebin uri=%s  ! videoscale ! video/x-raw,width=640,height=480 !videoconvert ! video/x-raw,format=BGR ! appsink sync=false name=\"sink\"",filename);
  sprintf(pipeline,"uridecodebin uri=%s  ! video/x-raw ! appsink sync=true name=\"sink\"",filename);
...
//  g_object_set (G_OBJECT (src), "uri", filename, NULL);
//  gst_object_unref (src);
  ...
  //pthread_create (&dec->gst_thread, NULL, csc_loop, dec);
  return dec;
}



char rgbbuffer[1920*1120*3];
char yuvbuffer[1920*1120*3];
...
buffer_to_file (struct decoder *dec, GstBuffer * buf)
{
 ...
 //   for (i = 0; i < nplanes; i++) {
  ...
  if(dec->frame%1==0){
	  convert(gst_dmabuf_memory_get_fd(mem));
//	  char *p=convert_get_picture();
//
//	  if(p){
////		  FILE *fp =fopen(filename,"w");
////		   fwrite(p,1,640*1183*3,fp);
////
////		   fclose(fp);
//		   convert_release_picture(p);
//	  }
}
...
static struct decoder *dec = NULL;
...
gint gstreamer_start (float ratio,gint argc, gchar * argv[])
{

	convert_boot_init(ratio);
...
int gstreamer_state(GstState *state)
{
	if(dec==0){
		*state=GST_STATE_NULL;
		return 0;
	}
	return gst_element_get_state (dec->pipeline,state,NULL,1*1000*1000);
}
