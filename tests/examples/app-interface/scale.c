...
int convert_init(void) {
	...
	resolution[0].total_width=items_width=resolution[1].width;//resolution[MAX_RESOLUTION_NUM-1].total_width+(16-resolution[MAX_RESOLUTION_NUM-1].total_width%LOCAL_ITEM_SIZE);
	resolution[0].total_height=items_height=resolution[MAX_RESOLUTION_NUM-1].total_height+(16-resolution[MAX_RESOLUTION_NUM-1].total_height%LOCAL_ITEM_SIZE);
...
// 		return -1;
// 		}
    program=CreateProgram(context,device_id,"scalergb.cl");
    ...
    /* Allocate memory for the input image that can be accessed by the CPU and GPU. */
 		...
 		gpu_yuv = clCreateImage2D(context, CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR, &format, resolution[0].width, resolution[0].height, 0, NULL, &ret);
 		if(ret!=CL_SUCCESS)
...
#define USEMMAP
int scale(char *cpu_yuv,char *cpu_rgb)
{

   struct timeval  start,end;
    cl_int ret;

//   ret = clSetKernelArg(kernel, 2, sizeof(cl_int), &srcwidth);
//   ret = clSetKernelArg(kernel, 3, sizeof(cl_int), &srcheight);
   ...
   gpu_cpu_yuv=clEnqueueMapImage(command_queue,  gpu_yuv, CL_TRUE, CL_MAP_WRITE, origin, region, &rowPitch, NULL, 0, NULL, NULL, &ret);
    //printf("\ngpu_cpu_yuv=%p\n",gpu_cpu_yuv);
   if (ret != CL_SUCCESS) {
    ...
    memcpy(gpu_cpu_yuv,cpu_yuv, resolution[0].width*resolution[0].height*3);
...
int main()
{
    ...
    FILE *fp=fopen("rgb.raw","r");
    fread(host_arr,1,NUM_ELEMENTS,fp);
    fclose(fp);
   while(1)
    {
    	scale(host_arr,host_arr);
    }
	fp=fopen("rgb.test","w");
